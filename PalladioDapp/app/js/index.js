...
$(document).ready(function() {
  ...
  $("#blockchain button.set").click(function() {
    ...
    if (EmbarkJS.isNewWeb3()) {
      palladio.methods.set(value).send({from: web3.eth.defaultAccount});
      addToLog("#blockchain", "palladio.methods.set(value).send({from: web3.eth.defaultAccount})");
    } else {
      palladio.set(value);
      addToLog("#blockchain", "palladio.set(" + value + ")");
    }
  ...
  $("#blockchain button.get").click(function() {
    // If web3.js 1.0 is being used
    if (EmbarkJS.isNewWeb3()) {
      palladio.methods.get().call(function(err, value) {
        $("#blockchain .value").html(value);
      });
      addToLog("#blockchain", "palladio.methods.get(console.log)");
    } else {
      palladio.get().then(function(value) {
        $("#blockchain .value").html(value.toNumber());
      });
      addToLog("#blockchain", "palladio.get()");
    }
  });
